...
- Endpoints sob `/admin/*` restritos exclusivamente a usuarios com role `admin` (usuarios com role `editor` nao possuem mais acesso administrativo).
...
- Seeder de desenvolvimento agora faz upsert do arquivo base por `external_file_id` com `withTrashed()`, evitando violacao de unicidade em reexecucoes quando o registro ja existe (inclusive soft-deletado).
- Seeder de desenvolvimento expandido para gerar alto volume de dados relacionais no banco (publicacoes longas e posts de timeline de todos os tipos, tags, comentarios, curtidas, salvos, visualizacoes e rede de seguidores), com limpeza e recriacao controlada dos registros `dev-*` para manter consistencia entre reexecucoes.
